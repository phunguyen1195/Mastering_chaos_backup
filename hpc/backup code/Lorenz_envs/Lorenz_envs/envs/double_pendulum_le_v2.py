...
class AcrobotEnv_origin(core.Env):
    """
    ### Description
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.state = self.np_random.uniform(low=np.array([np.pi-0.1,-0.1,-4,-4]), high=np.array([np.pi+0.1,0.1,4,4]), size=(4,)).astype(
            np.float32
        )
    ...
    def step(self, a):
        ...
        terminated = False
        reward = np.linalg.norm(ns[0]-self.goal[0])**2 + 0.001 * np.linalg.norm(ns[1]-self.goal[1])**2 + 0.001 * np.linalg.norm(ns[2]-self.goal[2])**2 + 0.001 * np.linalg.norm(ns[3]-self.goal[3])**2 + 0.001 * torque**2
        reward = reward[0]
        if self.render_mode == "human":
            self.render()
        return self._get_ob(), -reward, terminated, {'trajectory': s, 'action': a}
