...
class AcrobotEnv_origin(core.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        l2y = np.linspace(-9 * pi,  9 * pi, 101)
        l1x = np.linspace(-np.pi, np.pi, 101)
        l1y = np.linspace(-4 * pi,  4 * pi,101)
        X = np.array(np.meshgrid(l1x,l1y,l2x,l2y))


        self.precal_points = X.T.reshape(X.T.shape[0]*X.T.shape[1]*X.T.shape[2]*X.T.shape[3],4)
        # self.precal_le_  = np.load("precal_dp_18.npy")
        # self.precal_points = np.load("precal_dp_points_18.npy")
    ...
    def precal_le(self, s):
        ...
        # s_transformed = np.array([s[0],s[2],s[1],s[3]])
    ...
    def step(self, a):
        ...
        if self.render_mode == "human":
            self.render()
        return self._get_ob(), reward, terminated, {'trajectory': np.array([s[0]-np.pi/2,s[2],s[1],s[3]]), 'action': a}
