...
class Lorenzu(gym.Env):
    ...
    def sphere_reward (self, state,velocity ,action):
        ...
        if np.linalg.norm(state - self.goal_state) ** 2 - self.sphere_R ** 2 <= 0.0:
            ...
            # r = -self.heuristic_reward_8 ( state, velocity, action)
            r = 0
    ...
    def step(self, u):
        ...
        self.cost = self.sphere_reward(x,v,u)
    ...
    def reset(self):
        ...
        if self.close_to_goal == True:
            high = np.array([self.goal_state[0] + 20,self.goal_state[1]+ 20,self.goal_state[2]+ 20], dtype=np.float32)
            low = np.array([self.goal_state[0]- 20,self.goal_state[1]- 20, self.goal_state[2]- 20], dtype=np.float32)  # We enforce symmetric limits.
            # print ('high:', high )
            # print ('low:', low )
