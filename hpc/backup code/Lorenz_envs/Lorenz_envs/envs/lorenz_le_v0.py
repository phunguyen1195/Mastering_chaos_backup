...
class Lorenzle(gym.Env):
    ...
    def sphere_reward (self, state,new_state,action):
        # penalizing if out of radius
        if np.linalg.norm(state - self.goal_state) ** 2 - self.sphere_R ** 2 > 0.0:
            r = -30
        else:
            # r = -self.heuristic_reward_8 ( state, new_state, action)
    ...
    def le_reward (self, s , action):
        ...
        for i in range(1,T):
            ...
            x[:, i], _ = self.RungeKutta(self.dyn, self.lorenz, self.dt, x[:, i-1], np.array([0,0,0]))
    ...
    def goal_specific_le (self, state, velocity, action):
        avoid_state = np.array([0,0,0])
        cost = self.le_reward(state, action)
        # clip reward, should not be huge, twice value of le at positive equi
        cost_to_goal = np.linalg.norm(state - avoid_state) ** 2 + 0.1 * (np.linalg.norm(velocity)**2) + 0.01 * (np.linalg.norm(action)**2)
        cut_cost = np.linalg.norm((self.goal_state + 1) - avoid_state) ** 2 + 0.1 * (np.linalg.norm(velocity)**2) + 0.01 * (np.linalg.norm(action)**2)
        if cost_to_goal > cut_cost:
            cost_to_goal = cut_cost
        cost_to_goal = 1 * cost_to_goal
        return cost + cost_to_goal
    ...
    def step(self, u):
        ...
        u = np.clip(u, self.action_range_low, self.action_range_high)
        # self.action_u = u
        dyn = self.dyn
        f = self.lorenz
        dt = self.dt
        ...
        # no penalized for out of bound goal
        self.cost = self.le_reward(x, u)+ 0.001 * (np.linalg.norm(u)**2)

        # add le with avoiding specific goal
        # self.cost = self.goal_specific_le(x, v, u)
        ...
        # kill trajectory if too far from goal
        terminated = False
        if self.infinite == False:
            terminated = np.linalg.norm(x - self.goal_state) >= 3
