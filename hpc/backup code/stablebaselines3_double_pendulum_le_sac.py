...
# env = gym.make("double_pendulum_le-v0")
...
class SummaryWriterCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # self.v.append(self.locals['infos'][0]['trajectory'])
        self.a.append(self.locals['infos'][0]['action'])
        #print (self.n_calls)
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # print('state:', coordinate_list)
            print(self.first_link)
            self.logger.record("eval/mean angle of first link", np.mean(np.abs(self.first_link)))
            self.logger.record("eval/mean angle of second link", np.mean(np.abs(self.second_link)))
...
model.learn(total_timesteps=90000, callback=eval_callback)

model.save("models_for_paper/double_pendulum/nov11/sac_double_pendulum_withtermination_0")
