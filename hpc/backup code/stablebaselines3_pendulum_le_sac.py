...
class SummaryWriterCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            self.logger.record("eval/mean angle to goal", np.mean(coordinate_list[:,0]))
            self.logger.record("eval/mean angle velocity", np.mean(coordinate_list[:,1]))
...
model = SAC('MlpPolicy', env ,gamma=0.985, 
    # use_sde=True,
    # sde_sample_freq=4,
...
model.save("models_for_paper/Pendulum/sac_pendulum_le_24")
